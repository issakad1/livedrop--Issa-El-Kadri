...
r.get("/daily-revenue", wrapAsync(async (req, res) => {
  ...
  const pipeline = [
    ...
    { $project: { _id: 0, date: "$_id", revenue: { $round: ["$revenue", 2] }, orderCount: 1 } },
    { $sort: { date: 1 } }
  ];
...
r.get("/dashboard-metrics", wrapAsync(async (_req, res) => {
  const pipeline = [
    { $group: { _id: null, revenue: { $sum: "$total" }, count: { $sum: 1 }, avg: { $avg: "$total" } } },
    { $project: { _id: 0, revenue: { $round: ["$revenue", 2] }, orders: "$count", avgOrderValue: { $round: ["$avg", 2] } } }
  ];
  const [agg] = await orders.aggregate(pipeline);
